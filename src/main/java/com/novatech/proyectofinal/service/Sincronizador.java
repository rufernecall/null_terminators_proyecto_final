...
import com.novatech.proyectofinal.dao.LocalCacheManager;
...
public class Sincronizador {
    ...
    private void sincronizarClientes(Connection con) throws SQLException {
        List<com.novatech.proyectofinal.model.Cliente> lista = clienteDAO.listar();
        if (lista == null)
            return;
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            for (com.novatech.proyectofinal.model.Cliente c : lista) {
    ...
    private void sincronizarCategorias(Connection con) throws SQLException {
        List<com.novatech.proyectofinal.model.Categoria> lista = categoriaDAO.listar();
        if (lista == null)
            return;
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            for (com.novatech.proyectofinal.model.Categoria c : lista) {
                ps.setLong(1, c.getId());
                ps.setString(2, c.getNombre());
                ps.addBatch();
            }
