...
import com.null_terminators.proyectofinal.dao.LocalCacheManager;
...
public class Sincronizador {
    ...
    private void sincronizarProductos(Connection con) throws SQLException {
        ...
        if (lista == null)
            return; // Evita NullPointerException si falla la DB

        try (Statement st = con.createStatement()) {
            st.execute("DELETE FROM productos_cache");
        }
    ...
    private void sincronizarClientes(Connection con) throws SQLException {
        List<com.null_terminators.proyectofinal.model.Cliente> lista = clienteDAO.listar();
        if (lista == null)
            return;

        try (Statement st = con.createStatement()) {
            st.execute("DELETE FROM clientes_cache");
        }
        String sql = "INSERT INTO clientes_cache (id, nombre, documento, email) VALUES (?, ?, ?, ?)";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            for (com.null_terminators.proyectofinal.model.Cliente c : lista) {
    ...
    private void sincronizarCategorias(Connection con) throws SQLException {
        List<com.null_terminators.proyectofinal.model.Categoria> lista = categoriaDAO.listar();
        if (lista == null)
            return;

        try (Statement st = con.createStatement()) {
            st.execute("DELETE FROM categorias_cache");
        }
        String sql = "INSERT INTO categorias_cache (id, nombre) VALUES (?, ?)";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            for (com.null_terminators.proyectofinal.model.Categoria c : lista) {
                ps.setLong(1, c.getId());
                ps.setString(2, c.getNombre());
                ps.addBatch();
            }
